  ...
  return (
    <div className="header__content">
      <div className="header__content-left">
        ...
        <p>
          Город: {weatherData?.local_names?.ru ?? weatherData?.local_names?.en}
        </p>
        <img
          src={
            changeIcon(weatherData?.current.weather[0]?.main) ||
      ...
      <div className="header__content-right">
        <ul className="header__content-right-list">
          <li>
            ...
            <h3>Температура</h3>
            <p>
              {Math.floor(weatherData?.current.temp)}° - ощущается как{' '}
          ...
          <li>
            ...
            <h3>Давление</h3>
            <p>
              {Math.round(weatherData?.current.pressure * 0.7506)} мм ртутного
              столба - {pressureLvl(weatherData?.current.pressure * 0.7506)}
            </p>
          </li>
          <li>
            <div>
          ...
          <li>
            ...
            <h3>Ветер</h3>
            <p>
              {Math.round(weatherData?.current.wind_speed)} м/с{' '}
              {windDeg(weatherData?.current.wind_deg)} -{' '}
              {windStr(weatherData?.current.wind_speed)} ветер
            </p>
          </li>
        </ul>
      </div>
